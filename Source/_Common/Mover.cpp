...
//	m_nCollectOwnCnt = 0;
	...
	ZeroMemory(m_abHideCoat, sizeof(m_abHideCoat));
#endif
}
...
#ifdef __HIDE_FASHION
			if (j < 4 && m_abHideCoat[j])
			{
				BOOL bHide = FALSE;
				if (IsActiveMover())
					bHide = (GetEquipItem(nArryEquip1[j]) != NULL);
				else
					bHide = (m_aEquipInfo[nArryEquip1[j]].dwId != NULL_ID);

				if (bHide)
				{
					pModel->SetEffectOff(nArryEquip1[j], XE_HIDE);
					pModel->SetEffect(nArryEquip2[j], XE_HIDE);
#ifdef __HIDE_FASHION
					if (j == 0)
					{
						CItemElem* itemElem = NULL;
						if (IsActiveMover())
							itemElem = GetEquipItem(PARTS_CAP);

						if (itemElem)
						{
							if (itemElem->GetProp()->dwBasePartsIgnore != -1)
							{
								pModel->SetEffect(itemElem->GetProp()->dwBasePartsIgnore, XE_HIDE);
							}
						}
						else {
							...
							if (dwID)
							{
								...
								if (pProp)
								{
									if (pProp->dwBasePartsIgnore != -1)
										pModel->SetEffect(pProp->dwBasePartsIgnore, XE_HIDE);
								}
							}
						}
					}
				...
				{
					pModel->SetEffect(nArryEquip1[j], XE_HIDE);
					pModel->SetEffectOff(nArryEquip2[j], XE_HIDE);
			}
		}
			else
#endif // __HIDE_FASHION
