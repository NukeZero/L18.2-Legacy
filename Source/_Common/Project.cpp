...
#if defined (__CHARACTER_TAG)
			else if( script.Token == "TAG")
			{
				script.GetToken(); //(
				script.GetToken(); //tag
				_tcscpy( lpCharacter->strTagCharacter, script.token );	
				script.GetToken(); //,				
				lpCharacter->dwColorTag = script.GetNumber();
				lpCharacter->bTagCharacter = TRUE;
				script.GetToken(); // )
