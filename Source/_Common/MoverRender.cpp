...
void CMover::RenderName( LPDIRECT3DDEVICE9 pd3dDevice, CD3DFont* pFont, DWORD dwColor )
{
...
#endif
	{
		...
		{				
			LPCHARACTER lpCharacter = GetCharacter();
#ifndef __CHARACTER_TAG
			if( lpCharacter && lpCharacter->m_nStructure != -1 ) 
#else
			if( lpCharacter && lpCharacter->bTagCharacter == TRUE ) 
#endif
			{
				TCHAR szStr[ 64 ];
#if defined (__CHARACTER_TAG)
				DWORD colorTAG = lpCharacter->dwColorTag;
				sprintf( szStr, "[%s]", lpCharacter->strTagCharacter);
#else
				...
				point.y -= 35;
#if defined (__CHARACTER_TAG)
				pFont->DrawText( (FLOAT)( point.x ), (FLOAT)( point.y ), colorTAG, szStr );
#else
