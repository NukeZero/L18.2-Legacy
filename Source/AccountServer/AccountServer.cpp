...
#ifdef __SERVERS_INI_CONFIG
	if (Script(SERVER_CONFIG_FILE, SERVER_INI_ACCOUNT) == FALSE)
#else
...
//		OutputDebugString( "EnableConnectionPooling\n" );
	...
	if( g_dpSrvr.LoadAddrPmttd(SERVER_ALWIP_FILE) == false )
	{
		TRACE("LoadAddrPmttd false\n");
	}	
	
	if( g_dpSrvr.LoadIPCut(SERVER_BANIP_FILE) == FALSE )
	{
		...
		case WM_COMMAND:
			...
			switch (wmId)
			{
				...
				case IDM_OPT_INTERNAL_IP:
					{
						g_dpSrvr.LoadAddrPmttd(SERVER_ALWIP_FILE);
						g_dpSrvr.LoadIPCut(SERVER_BANIP_FILE);
						break;
					}
				default:
