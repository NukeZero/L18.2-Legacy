...
void CWndConfirmBuy::OnOK()
{
...
#endif
	{
		dwCost = m_pItemElem->GetChipCost();
#ifdef __VENDOR_UPDATE
			if( m_pItemElem->m_nItemNum < 1 || dwCost == 0 || m_pItemElem->GetProp()->dwReferValue1 < 1 ) 
#else
...
#endif
		{
			g_WndMng.OpenMessageBox( _T("More %s are needed.", prj.GetItemProp(lpCharacter)->szName ) );	// 칩이 부족합니다.	
			return;
		}
	}
	else if(m_nBuyType == 0)
...
//		CItemBase* pItemBase	= pItemCtrl->m_pArrayItemBase[nIndex];
	...
	if( lpCharacter && lpCharacter->m_nVenderType != 0 )
		strName.Format( prj.GetItemProp(lpCharacter->m_nVenderType)->szName );
	else
		strName.Format( "Penya" );
	
	/*
	CWndStatic* pStaticName = (CWndStatic*)GetDlgItem( WIDC_STATIC5 );
	pStaticName->SetTitle( strName );
	*/
...
void CWndShop::OnMouseWndSurface( CPoint point )
{
	...
	if( pCtrl && pCtrl->rect.PtInRect( point ) )
	{
		...
		if( lpCharacter )
		{
			...
			ClientToScreen(&p2);
			ClientToScreen(&Draw);
			g_WndMng.PutToolTip_Item( (CItemBase*)&pItem, p2, &Draw );
		}
	}
}
#endif
