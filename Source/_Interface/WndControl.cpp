		...
		for (unsigned i = nPos; i < nEnd; ++i)
		{
			...
			for (unsigned j = 0; j < pSet->vecSet[i].vecCondItem.size(); j++)
			{
				...
				if (pPropItem)
				{
					...
					m_p2DRender->TextOut(pt.x + 29 - size.cx, pt.y + 31 - size.cy + 4, szTemp, 0xFF0099ff);
					p2DRender->SetFont(pOldFont);
			...
			for (unsigned j = 0; j < pSet->vecSet[i].vecPayItem.size(); j++)
			{
				...
				if (pPropItem)
				{
					...
					m_p2DRender->TextOut(pt.x + 29 - size.cx, pt.y + 29 - size.cy + 4, szTemp, 0xFF0099ff);
