...
#ifdef __HIDE_FASHION
						if (pItemProp->dwBasePartsIgnore == PARTS_HAIR && pMover->m_abHideCoat[0] == 0)
#endif //__HIDE_FASHION
							((CModelObject*)m_pModel)->SetEffect(pItemProp->dwBasePartsIgnore, XE_HIDE);
#ifdef __HIDE_FASHION
						else
							pElement = m_pModel->SetEffectOff(pItemProp->dwBasePartsIgnore, XE_HIDE);
#endif //__HIDE_FASHION
					}
				}
			}
...
void CWndInventory::OnDraw(C2DRender* p2DRender)
{
	...
	if (pWndCheck) 
	{
		pWndCheck->SetCheck(g_Option.m_bViewMask ? TRUE : FALSE);
	}
	...
	for( int i=2; i<MAX_HUMAN_PARTS; i++ )
	{
		...
		if (pItemBase && pItemBase->GetTexture())
		{
			...
			else if (i >= PARTS_HAT && i <= PARTS_BOOTS)
			{
				...
				if (g_pPlayer->m_abHideCoat[i - PARTS_HAT])
					dwAlpha = 100;
			}
...
void CWndInventory::OnInitialUpdate()
{
...
#ifdef __UI_INVENTORYSORT
	sortmenu.CreateMenu(this);
	...
	if (pWndCheck)
	{
		pWndCheck->SetCheck(g_Option.m_bViewMask);
		CPoint pt = m_InvenRect[PARTS_MASK].TopLeft();
		pWndCheck->Move(pt);
	}
#endif // __HIDE_FASHION
...
BOOL CWndInventory::OnChildNotify( UINT message, UINT nID, LRESULT* pLResult )
{
	...
	{
		CWndButton* pWndCheck = (CWndButton*)GetDlgItem(WIDC_CHECK2);
		g_Option.m_bViewMask = pWndCheck->GetCheck();
		g_DPlay.SendOptionEnableRenderMask(pWndCheck->GetCheck());
	}
#endif

#ifdef __UI_INVENTORYSORT
#define MIS_SORT_NAMEASC	0
#define MIS_SORT_NAMEDESC	1
#define MIS_SORT_IDASC		2
#define MIS_SORT_IDDESC		3

	if (nID == WIDC_BUTTON1)
	{
		sortmenu.DeleteAllMenu();
		sortmenu.AppendMenu(0, MIS_SORT_NAMEASC, "Type Ascending");
		sortmenu.AppendMenu(0, MIS_SORT_NAMEDESC, "Type Descending");
		sortmenu.AppendMenu(0, MIS_SORT_IDASC, "ID Ascending");
		sortmenu.AppendMenu(0, MIS_SORT_IDDESC, "ID Descending");
		sortmenu.Move(CPoint(m_rectCurrentWindow.left, m_rectCurrentWindow.top) + GetMousePoint());
		...
		if( pWndFrame && nID == 11 ) // item
		{
			...
			if( pWndFrame->GetWndId() == APP_SHOP_ )
			{
				...
				if(pwndShop)
				{
					...
					{
							if (g_pPlayer->m_Inventory.GetAtItemNum(lpCharacter->m_nVenderType) - (int)((CItemElem*)lpShortcut->m_dwData)->GetChipCost() >= 0) //구매하려는 품목의 칩개수 이상을 가지고 있는지 확인.
							{
								SAFE_DELETE( m_pWndConfirmBuy );
							...
							{
								CString str;
								str.Format(_T("More %s are needed."), prj.GetItemProp(lpCharacter->m_nVenderType)->szName);
...
#ifdef __UI_INVENTORYSORT
bool sortinven_idasc(const sItem& a, const sItem& b)
{
	return a.dwItemId < b.dwItemId;
}
bool sortinven_iddesc(const sItem& a, const sItem& b)
{
	return a.dwItemId > b.dwItemId;
}
bool sortinven_nameasc(const sItem& a, const sItem& b)
{
	return a.dwKind2 < b.dwKind2;
}
bool sortinven_namedesc(const sItem& a, const sItem& b)
{
	return a.dwKind2 > b.dwKind2;
}
#endif
...
{
#ifdef __UI_INVENTORYSORT
	if (pWndBase == &sortmenu)
	{
		sortmenu.SetVisible(FALSE);

		CWndInventory* pWndInventory = (CWndInventory*)GetWndBase(APP_INVENTORY);
		CWndTabCtrl* pTabCtrl = (CWndTabCtrl*)pWndInventory->GetDlgItem(WIDC_INVENTORY);
		CHAR cTab = (CHAR)pTabCtrl->GetCurSel();
		BOOL bKey = GetAsyncKeyState(VK_SHIFT) >> ((sizeof(SHORT) * 8) - 1);
		...
		switch (nID)
		{
		case MIS_SORT_NAMEASC:
			sort(vItem.begin(), vItem.end(), sortinven_nameasc);
			break;
		case MIS_SORT_NAMEDESC:
			sort(vItem.begin(), vItem.end(), sortinven_namedesc);
			break;
		case MIS_SORT_IDASC:
			sort(vItem.begin(), vItem.end(), sortinven_idasc);
			break;
		case MIS_SORT_IDDESC:
		default:
			sort(vItem.begin(), vItem.end(), sortinven_iddesc);
			break;
		}
