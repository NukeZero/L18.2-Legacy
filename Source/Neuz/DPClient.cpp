...
//			case SNAPSHOTTYPE_STOP_COLLECT:		OnStopCollect( objid, ar );	break;
		...
		case SNAPSHOTTYPE_HIDE_COAT: OnHideCoat(ar, objid); break;
#endif
...
void CDPClient::OnJoin( CAr & ar )
{
	...
	if (g_pPlayer)
	{
		if (g_pPlayer->IsMode(MODE_OPTION_DONT_RENDER_MASK))
		{
			g_Option.m_bViewMask = FALSE;
		}
		else
		{
			g_Option.m_bViewMask = TRUE;
		}
	}
#endif
}
...
#ifdef __LDE_WELCOME_NOTICE
	strMessage.Format(_T(WELCOME_NOTICE_01));
	g_WndMng.PutString(strMessage, NULL, COLOR_NOTICE, CHATSTY_HELP);

	strMessage.Format(_T(WELCOME_NOTICE_02));
	g_WndMng.PutString(strMessage, NULL, COLOR_NOTICE, CHATSTY_HELP);
#endif
}
...
void CDPClient::OnHideCoat(CAr& ar, OBJID objid)
{
	int nPosition;
	BOOL bHide;
	ar >> nPosition >> bHide;
	...
	CMover* pMover = prj.GetMover(objid);
	if (pMover)
		pMover->m_abHideCoat[nPosition] = (bHide == 1);
}
#endif
