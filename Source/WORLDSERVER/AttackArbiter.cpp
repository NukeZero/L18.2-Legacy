...
int CAttackArbiter::PostCalcDamage( int nATK, ATTACK_INFO* pInfo )
{
  ...
  if( pDefender->IsPlayer() && pAttacker->IsPlayer() )
  {
	...
	if (pInfo->pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_FIRE_RATE, 0) && pDefender->GetProp()->eElementType == SAI79::FIRE)
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_FIRE_RATE, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_WATER_RATE, 0) && pDefender->GetProp()->eElementType == SAI79::WATER)
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_WATER_RATE, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_ELECT_RATE, 0) && pDefender->GetProp()->eElementType == SAI79::ELECTRICITY)
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_ELECT_RATE, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_WIND_RATE, 0) && pDefender->GetProp()->eElementType == SAI79::WIND)
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_WIND_RATE, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_EARTH_RATE, 0) && pDefender->GetProp()->eElementType == SAI79::EARTH)
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_PVE_DMG_ELEMENT_EARTH_RATE, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_STUN, 0) && pDefender->IsStateMode(CHS_STUN))
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_STUN, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_DARK, 0) && pDefender->IsStateMode(CHS_DARK))
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_DARK, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_POISON, 0) && pDefender->IsStateMode(CHS_POISON))
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_POISON, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_SLOW, 0) && pDefender->IsStateMode(CHS_SLOW))
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_SLOW, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_BLEEDING, 0) && pDefender->IsStateMode(CHS_BLEEDING))
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_BLEEDING, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_SILENT, 0) && pDefender->IsStateMode(CHS_SILENT))
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_SILENT, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_LOOT, 0) && pDefender->IsStateMode(CHS_LOOT))
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_LOOT, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_SETSTONE, 0) && pDefender->IsStateMode(CHS_SETSTONE))
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_SETSTONE, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}

	if (pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_SLEEPING, 0) && pDefender->IsStateMode(CHS_SLEEPING))
	{
		fRatio = pInfo->pAttacker->GetParam(DST_GIVE_DMG_RATE_ENEMY_SLEEPING, 0) / 100.0f;
		nDamage = (int)(nDamage + (nDamage * fRatio));
		fRatio = 0.0f;
	}
#endif
