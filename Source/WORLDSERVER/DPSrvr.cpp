...
#else
		if (lpChar && lpChar->m_nVenderType != 1)	// 1 - ? ??
#endif
			return;
	...
	void CDPSrvr::OnFashionCombine(CAr& ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long)
	{
		...
		if (IsValidObj(pUser))
		{
			...
			if (xRandom(10000) < 10000)
			{
				...
				pUser->AddText("You have succesfully combined the blessings on your fashion!", COLOR_SUCCESS);
			...
			else
			{
				...
				if (pUser->HasBuff(BUFF_ITEM, II_SYS_SYS_SCR_COSTUMEDEFENDER))
				{
					...
					pUser->AddText("Fashion Combination has failed, but your items have been protected.", COLOR_WARNING);
				}
				else
				{
					pUser->AddText("Fashion Combination Failed!  All items have been destroyed.", COLOR_ERROR);
		...
		case 1:
			...
			if (pParty && pParty->IsLeader(pUser->m_idPlayer))
			{
				if (CColosseumMng::GetInstance()->CreateNewMatch(pUser, byDifficult, byMenu))
				{
					...
					pUser->AddText("Entry is currently unavailable. Please try again later.", COLOR_ERROR);
				}
				else
					pUser->AddText("Entry is currently unavailable. Please try again later.", COLOR_ERROR);
			}
			else
				pUser->AddText("You must be in a party to enter the Colosseum.", COLOR_ERROR);
			break;
		case 2:
			if (pParty)
			{
...
void CDPSrvr::OnItemLink(CAr& ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long)
{
	...
	if (IsValidObj(pUser))
	{
		...
		{
			pUser->AddText("That player is not connected.", COLOR_ERROR);
			return;
		}
		CItemElem* pItemElem = pOther->m_Inventory.GetAtId(dwObjId);
		if (pItemElem == NULL)
		{
			pUser->AddText("This item has expired.", COLOR_ERROR);
			return;
		}
...
BOOL CDPSrvr::UnbindScroll(CUser* pUser, CItemElem* pMaterial, CItemElem* pTarget)
{
	...
	if (pTarget->IsFlag(CItemElem::binds) && pMaterial->m_dwItemId == II_SYS_SYS_SCR_CANCELLATION && (pTarget->GetProp()->dwFlag & IP_FLAG_EQUIP_BIND))
	{
		...
		log.Gold2 = pUser->GetGold();
		OnLogItem(log, pTarget, 1);
		pUser->AddText("Your item has been successfully unbound.", COLOR_SUCCESS);
		return TRUE;
	}
	else
		pUser->AddText("This scroll can only be used on most no-trade, soul linked or event items.", COLOR_ERROR);
	return FALSE;
}
#endif
...
void CDPSrvr::OnRandomOptionRetry(CAr& ar, DPID dpidCache, DPID dpidUser, LPBYTE, u_long)
{
	...
	if (IsValidObj(pUser) == TRUE)
	{
		...
		if (IsUsableItem(pItemElem) && IsUsableItem(pItemElem2))
		{
			...
			if (pUser->m_Inventory.IsEquip(pItemElem->m_dwObjId))
			{
				pUser->AddText("Please unequip the item first.", COLOR_ERROR);
				return;
			}

			if (pUser->HasActivatedSystemPet() && pItemElem->GetProp()->dwItemKind3 == IK3_EGG)
			{
				pUser->AddText("You can't change awakening when a Pet is summoned.", COLOR_ERROR);
				return;
			}
			...
			if (nRandomOptionKind == CRandomOptionProperty::eAwakening)
			{
				...
				if (nSize >= 2 && nSize <= 3)
				{
					...
					pUser->AddText("Awakening has been successfully changed.", COLOR_SUCCESS);
				}
			}
		}
	}
