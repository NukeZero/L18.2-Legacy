...
*/
...
#ifdef __SERVERS_AUTHORIZE
		char strAccountFile[16];
		char strNameFile[16];
		char lpAddrFile[64];
		DWORD strAuth = AUTH_GENERAL;
		CScanner s;

		if (s.Load(SERVER_AUTHS_FILE))
		{
			pUser->m_nAuth = 0;
			s.GetToken();
			while (s.tok != FINISHED)
			{
				if (s.Token == "Authorization")
				{
					s.GetToken();
					if (s.Token == "{")
					{
						...
						while (s.Token != "}")
						{
							...
							if (s.Token == "Account" || s.Token == "account" || s.Token == "ACCOUNT")
							{
								CString strAccount = pUser->m_playAccount.lpszAccount;
								s.GetToken();
								strcpy(strAccountFile, s.Token);
								s.GetToken();
								DWORD strAuth = (DWORD)s.Token.GetBuffer(s.Token.GetLength());
								if (strAccountFile == strAccount)
								{
									if (s.Token == "AUTH_GAMEMASTER")
										pUser->m_dwAuthorization = AUTH_GAMEMASTER;
									else if (s.Token == "AUTH_GAMEMASTER2")
										pUser->m_dwAuthorization = AUTH_GAMEMASTER2;
									else if (s.Token == "AUTH_GAMEMASTER3")
										pUser->m_dwAuthorization = AUTH_GAMEMASTER3;
									else if (s.Token == "AUTH_OPERATOR")
										pUser->m_dwAuthorization = AUTH_OPERATOR;
									else if (s.Token == "AUTH_ADMINISTRATOR")
							...
							if (s.Token == "Ip" || s.Token == "ip" || s.Token == "IP")
							{
								CString lpAddr = pUser->m_playAccount.lpAddr;
								s.GetToken();
								strcpy(lpAddrFile, s.Token);
								s.GetToken();
								DWORD strAuth = (DWORD)s.Token.GetBuffer(s.Token.GetLength());
								if (lpAddrFile == lpAddr)
								{
									if (s.Token == "AUTH_GAMEMASTER")
										pUser->m_dwAuthorization = AUTH_GAMEMASTER;
									else if (s.Token == "AUTH_GAMEMASTER2")
							...
							if (s.Token == "Character" || s.Token == "character" || s.Token == "CHARACTER")
							{
								CString strName = pUser->GetName();
								s.GetToken();
								strcpy(strNameFile, s.Token);
								s.GetToken();
								DWORD strAuth = (DWORD)s.Token.GetBuffer(s.Token.GetLength());
								if (strNameFile == strName)
								{
									if (s.Token == "AUTH_GAMEMASTER")
										pUser->m_dwAuthorization = AUTH_GAMEMASTER;
									else if (s.Token == "AUTH_GAMEMASTER2")
		...
		{
			pUser->m_dwAuthorization = AUTH_GENERAL;
			Error("Authorization.ini not found");
		}
#endif
